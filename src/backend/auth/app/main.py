...
@auth.post("/register", response_model=schemas.UserResponse, status_code=status.HTTP_201_CREATED)
...
@auth.post("/login")
def login_user(payload: schemas.UserCreate, db: Session = Depends(get_db)) -> Dict[str, str]:
